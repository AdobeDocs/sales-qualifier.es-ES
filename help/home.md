---
title: Información general de Sales Qualifier
description: Obtenga información sobre Sales Qualifier, una aplicación basada en IA que automatiza la calificación de clientes potenciales, la divulgación y la participación del comprador para los equipos de ventas B2B.
feature: Agentic AI, Sales Insights, Account Journeys
role: User
TQID: 'https://experienceleague.adobe.com/4IsAlgFBxddHCz0-CLCDXPCHI7m3motiXhgCR6MZq0k'
product_v2:
  - id: d0a3eab4-7b10-4d96-a71e-6c0f8e7b7c87
feature_v2:
  - id: fc7979f3-56c3-43ca-9784-f1ea3dc69c4b
topic_v2:
  - id: aa2f3246-cb95-4b30-8899-fdf7d73550cc
  - id: bce87dde-a4ab-44c9-8a18-ad66e4ddb377
  - id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: 08dd05e1d13b501d43d457e6217a43aaabdb1d0d
workflow-type: tm+mt
source-wordcount: 412
ht-degree: 22%

---


# Calificador de ventas

Adobe Sales Qualifier es una aplicación con tecnología de IA que automatiza la calificación de clientes potenciales, la divulgación y la participación de compradores en canales múltiples para los representantes de desarrollo empresarial (BDR). Account Qualification Agent analiza sus cuentas y clientes potenciales, prioriza las que están listas para el siguiente paso y elabora un alcance personalizado basado en sus datos CRM.

Los BDR pueden utilizar los complementos de explorador y correo electrónico para acceder a inteligencia empresarial directamente desde su CRM o Outlook.

## Qué puede hacer en Sales Qualifier

* **Priorice clientes potenciales y cuentas** con resúmenes de actividad generados por IA y priorización basada en señales.
* **Cree planes de participación orientados a objetivos** en los que AI proponga una cadencia y redacte correos electrónicos personalizados para cada posible cliente.
* **Proximidad del proceso en una cola** para llamadas telefónicas, LinkedIn InMails y revisiones de correo electrónico.
* **Reserve reuniones automáticamente** conectando su calendario e insertando un enlace personal de reserva en su cadencia.
* **Informe al alcance de la IA con sus propios materiales** mediante la creación de un manual en el Centro de conocimientos.
* **Haga preguntas en lenguaje natural** con el chat de IA, que usa sus datos de CRM, participación y centro de conocimiento.
* **Rastree el rendimiento de las actividades de extensión** con informes de correo electrónico y de reserva de reuniones.

>[!VIDEO](https://video.tv.adobe.com/v/3476564?captions=spa)

## Explorar la guía

::::landing-cards-container
:::card
![icono](https://cdn.experienceleague.adobe.com/icons/circle-play.svg?lang=es)

Introducción

Complete la configuración de administrador única para grupos de usuarios y una conexión CRM.

[Introducción](getting-started.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/list-check.svg?lang=es)

Clientes potenciales

Genere, filtre y revise su lista de clientes potenciales.

[Clientes potenciales](prospects.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/list-check.svg?lang=es)

Cuentas

Priorice el alcance con los datos de participación y canalización a nivel de cuenta.

[Cuentas](accounts.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/code-branch.svg?lang=es)

Flujos de trabajo salientes

Cree cadencias dirigidas por objetivos con correos electrónicos personalizados generados por IA.

[Flujos de trabajo salientes](outbound-workflows.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/list-check.svg?lang=es)

Tareas

Procesar llamadas telefónicas, LinkedIn InMail y tareas de revisión de correo electrónico en una cola.

[Tareas](tasks.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/list-check.svg?lang=es)

Desempeño

Revisar el rendimiento de correo electrónico y reserva de reuniones.

[Rendimiento de salida](performance.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/list-check.svg?lang=es)

Centro de información

Cree un manual que apoye el alcance y la asistencia de la IA en sus materiales.

[Centro de información](knowledge-center.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/circle-play.svg?lang=es)

Integraciones

Conecte Salesforce o Microsoft Dynamics 365 y asigne los campos.

[Integraciones](integrations.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/list-check.svg?lang=es)

Configuración de perfil

Configure la disponibilidad del correo electrónico, la firma y el calendario.

[Configuración de perfil](profile-settings.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/list-check.svg?lang=es)

Funciones de usuario y permisos

Comprender el modelo de grupo de usuarios y administradores.

[Funciones de usuario y permisos](user-roles-permissions.md)
:::

:::card
![icono](https://cdn.experienceleague.adobe.com/icons/circle-play.svg?lang=es)

Chat de IA

Haga preguntas en función de sus datos de CRM, participación y centro de conocimiento.

[Chat de IA](ai-assistant.md)
:::

::::
